<?php
    ...
    $thumb_image_name = 'starto-gallery-grid';
	$column_class = 'tg-four-cols';
	$column_size = 4;
	
	$grid_template = get_theme_mod('starto_course_grid_template', 1);
	$hover_effect = get_theme_mod('starto_course_hover_effect', 1);
	$hover_class = '';
	if(empty($hover_effect))
	{
		$hover_class = 'nohover';
	}
	$show_rating = get_theme_mod('starto_course_rating', 1);
	$show_price = get_theme_mod('starto_course_pricing', 1);
	$show_tooltip = get_theme_mod('starto_course_tooltip', 1);
	$show_lesson = get_theme_mod('starto_course_lesson', 1);
	$show_student = get_theme_mod('starto_course_student', 1);
	$count = 0;
	...
	else if(is_single() && $post->post_type == 'lp_course')
	{
		...
		$starto_course_related_sort = get_theme_mod('starto_course_related_sort', 'default');
		
		switch($starto_course_related_sort)
		{
			case 'default':
		...
		$starto_course_related_item = get_theme_mod('starto_course_related_item', 4);
		
		//Start displaying columns
		switch($starto_course_related_item)
		{
			case 2:
		...
		$course_grid_header = esc_html__("More courses you might like", 'starto' );
	}
...
?>
		...
		?>
			...
			<?php
				if(!empty($show_price))
				{
					$course_price = starto_get_course_price_html($post_ID);
			?>
			...
			<div class="card-info">
				...
				<div class="tooltip-templates">
				    <div id="tooltip_content_<?php echo esc_attr($post_ID); ?>" class="course-tooltip-content">
					    <h5><?php esc_html_e("What you'll learn", 'starto' ); ?></h5>
				...
				<div class="card-meta-wrapper">
					<?php
					if(function_exists('starto_get_course_curriculum_number') && !empty($show_lesson))
					{
						$course_lessons = starto_get_course_curriculum_number($post_ID);
