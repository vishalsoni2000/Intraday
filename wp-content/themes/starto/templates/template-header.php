...
elseif(is_archive())
{
    ...
    elseif ( !empty($term) ) :
    	...
    	$page_title = esc_html__('Blog Archives', 'starto');
    endif;
...
{
	$page_show_title = 0;
	$page_title = esc_html__('Search', 'starto' );
	$term = 'search';
...
{
	$page_show_title = 0;
	$page_title = esc_html__('Blog', 'starto' );
	$term = 'home';
}

$starto_page_content_class = starto_get_page_content_class();

$starto_hide_title = starto_get_hide_title();
if($starto_hide_title == 1)
{
	$page_show_title = 1;
}

$starto_screen_class = starto_get_screen_class();
if($starto_screen_class == 'split' OR $starto_screen_class == 'single_client')
{
	$page_show_title = 0;
}
if($starto_screen_class == 'single_client')
{
	$page_show_title = 1;
}
...
if(empty($page_show_title))
{
	...
	{
		$page_tagline = esc_html__('Archive posts in ', 'starto' );
	...
	if(is_search())
	{
		$page_tagline = esc_html__('Search Results for ', 'starto' ).get_search_query();
	}
	...
	$page_header_type = '';
?>
<div id="page-header" class="<?php if(!empty($pp_page_bg) ) { ?>hasbg <?php if(!empty($starto_page_header_bg_parallax)) { ?>parallax<?php } ?> <?php } ?> <?php if(!empty($starto_topbar)) { ?>withtopbar<?php } ?> <?php if(!empty($starto_screen_class)) { echo esc_attr($starto_screen_class); } ?> <?php if(!empty($starto_page_content_class)) { echo esc_attr($starto_page_content_class); } ?>" <?php if(!empty($pp_page_bg)) { ?>style="background-image:url(<?php echo esc_url($pp_page_bg); ?>);"<?php } ?>>
	...
	<div class="page-title-wrapper">
		<div class="standard-wrapper">
			<div class="page-title-inner">
				<?php
					$starto_page_title_font_alignment = get_theme_mod('starto_page_title_font_alignment', 'left');
				?>
				<div class="page-title-content title_align_<?php echo esc_attr($starto_page_title_font_alignment); ?>">
					<h1 <?php if(!empty($pp_page_bg) && !empty($starto_topbar)) { ?>class ="withtopbar"<?php } ?>><?php echo esc_html($page_title); ?></h1>
					<?php
...
<!-- Begin content -->
<div id="page-content-wrapper" class="<?php if(!empty($page_boxed_layout)) { ?>blog-wrapper <?php } ?><?php if(!empty($pp_page_bg)) { ?>hasbg <?php } ?><?php if(!empty($pp_page_bg) && !empty($starto_topbar)) { ?>withtopbar <?php } ?><?php if(!empty($starto_page_content_class)) { echo esc_attr($starto_page_content_class); } ?>">
