...
$starto_shop_layout = get_theme_mod('starto_shop_layout', 'fullwidth');

if(STARTO_THEMEDEMO && isset($_GET['sidebar']))
{
	$starto_shop_layout = 'sidebar';
}

if($starto_shop_layout == 'sidebar')
{
...
if(empty($page_show_title))
{
    ...
    if(is_product())
    {
	    $shop_page_id = get_option('woocommerce_shop_page_id');
	    $page_title = get_the_title($shop_page_id);
	    $page_tagline = '<a href="'.esc_url(get_permalink($shop_page_id)).'"><span class="ti-arrow-left"></span>&nbsp;'.esc_html__('Back to shop', 'starto' ).'</a>';
