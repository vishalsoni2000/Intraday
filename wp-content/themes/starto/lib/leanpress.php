<?php
function starto_get_course_curriculum_number($course_id = '')
{
	$course_lessons = 0;
...
add_action( 'learn-press/before-single-course', 'starto_single_course_header' );
function starto_single_course_header() {
	$obj_post = starto_get_wp_post();
	...
	$starto_course_template = starto_get_single_course_template($obj_post->ID);
?>
<div id="single-course_wrapper" class="course_template_<?php echo esc_attr($starto_course_template); ?>">
...
<?php 
	...
	//Get course metadata
	$course_duration = starto_get_course_duration_string($obj_post->ID);
	$course_skill_level = get_post_meta($obj_post->ID, '_lp_skill_level', true);
	$course_lessons = '';
	
	if(function_exists('starto_get_course_curriculum_number'))
	{
		$course_lessons = starto_get_course_curriculum_number($obj_post->ID);
	}
	...
	$starto_course_include = get_theme_mod('starto_course_include');
...
<div id="single-course-header">
	<div class="standard-wrapper">
		<div class="single-course-title">
			...
			<span class="single-course_rating_total">
				...
				<?php
					if($total > 1)
					{
						echo esc_html_e('ratings', 'starto' );
					}
					else
					{
						echo esc_html_e('rating', 'starto' );	
					}
				?>)
			</span>
			<?php
		...
		<div class="single-course-join">
			<a id="single-course-enroll" href="<?php echo esc_js('javascript:;'); ?>" class="button"><?php esc_html_e('Enroll This Course', 'starto' ); ?></a>
		</div>
		<?php
			}
		?>
...
<div id="single-course-meta" class="standard-wrapper">
	<ul class="single-course-meta-data">
		...
		<li>
			...
			<div class="single-course-meta-data-text">
				<span class="single-course-meta-data-title">
					<?php esc_html_e('Duration', 'starto' ); ?>
				</span>
				<?php
		...
		<li>
			...
			<div class="single-course-meta-data-text">
				<span class="single-course-meta-data-title">
					<?php esc_html_e('Skill Level', 'starto' ); ?>
				</span>
				<span class="single-course-meta-data-content">
		...
		<li>
			...
			<div class="single-course-meta-data-text">
				<span class="single-course-meta-data-title">
					<?php esc_html_e('Lectures', 'starto' ); ?>
				</span>
				<span class="single-course-meta-data-content">
					...
					<?php
						if($course_lessons > 1)
						{
							echo esc_html_e('lessons', 'starto' );
						}
						else
						{
							echo esc_html_e('lesson', 'starto' );
						}
					?>
				</span>
			</div>
		...
		<li>
			...
			<div class="single-course-meta-data-text">
				<span class="single-course-meta-data-title">
					<?php esc_html_e('Enrolled', 'starto' ); ?>
				</span>
				<span class="single-course-meta-data-content">
					...
					<?php
						...
						{
							echo esc_html_e('students', 'starto' );
						}
						else
						{
							echo esc_html_e('student', 'starto' );
						}
					?>
				</span>
			</div>
			...
			<div class="standard-wrapper">
				...
				<div class="video-grid-wrapper">
					<a href="<?php echo esc_js('javascript:;'); ?>" class="video-card" data-video-id="<?php echo esc_attr($obj_post->ID); ?>">
						...
						<span class="preview-video-title"><?php esc_html_e('Preview this course', 'starto' ); ?></span>
					</a>
				...
				<div class="single-course-include">
					...
					<h4><?php esc_html_e('This course include', 'starto' ); ?></h4>
					...
					<ul class="single-course-include_list">
						...
						<li>
							...
							<span class="single-course-include_title">
								<?php esc_html_e('Duration', 'starto' ); ?> <?php echo esc_html($course_duration); ?>
							</span>
						</li>
						<?php
							}
						...
						<li>
							...
							<span class="single-course-include_title">
								<?php esc_html_e('Skill Level', 'starto' ); ?> <?php echo esc_html($course_skill_level); ?>
							</span>
						</li>
						<?php
							}
						...
						<?php
							foreach($starto_course_include as $starto_course_include_item)
							{
								$icon_thumb = wp_get_attachment_image_src($starto_course_include_item['course_include_icon'], 'full', true);
						?>
						<li>
							<?php
								...
								{
							?>
								<span class="single-course-include_icon"><img src="<?php echo esc_url($icon_thumb[0]); ?>" alt="<?php echo esc_attr($starto_course_include_item['course_include_title']); ?>"/></span>
							<?php
								}
							?>
							<span class="single-course-include_title"><?php echo esc_html($starto_course_include_item['course_include_title']); ?></span>
						</li>
						<?php
							}
						?>
...
add_action( 'learn-press/single-course-summary', 'starto_single_course_footer' );
function starto_single_course_footer() {
	$obj_post = starto_get_wp_post();
	if(class_exists('LP_Global'))
	{
		$obj_course = LP_Global::course();
	...
	$starto_course_related = get_theme_mod('starto_course_related', true);
	...
	switch($starto_course_template)
	{
		...
		case 4:
			...
			if(!empty($starto_course_related))
			{
...
<div class="single-course_sidebar <?php echo esc_attr($has_image_class); ?>">
	...
	<div class="single-course_info_wrapper">
		...
		<?php
			...
			if(!empty($starto_course_include))
			{
		?>
		<div class="single-course-include">
			<h4><?php esc_html_e('This course include', 'starto' ); ?></h4>
