<?php
add_filter( 'manage_posts_columns', 'starto_add_gravatar_col');
function starto_add_gravatar_col($cols) {
	$cols['thumbnail'] = esc_html__('Thumbnail', 'starto-elementor');
	return $cols;
}

add_action( 'manage_posts_custom_column', 'starto_get_author_gravatar');
function starto_get_author_gravatar($column_name ) {
...
$pages = get_pages();
$pages_select = array(
	'' => esc_html__('Blank Page', 'starto-elementor' )
);
...
$theme_course_templates = array(
	 '' => esc_html__('Use default course template', 'starto-elementor' ),
	  1	=> 'Style 1',
	  2	=> 'Style 2',
	  3	=> 'Style 3',
	  4	=> 'Style 4',
...
$postmetas = 
	array (
		'post' => array(
			array("id" => "post_layout", "type" => "select", "title" => esc_html__('Post Layout', 'starto-elementor' ), "description" => esc_html__('You can select layout of this single post page.', 'starto-elementor' ), "items" => $post_layout_select),
			
			array("id" => "hide_featured_image", "type" => "checkbox", "title" => esc_html__('Hide Featured Image', 'starto-elementor' ), "description" => esc_html__('Check this option if you want to hide post\'s featured image ', 'starto-elementor' )),
		),
		'lp_course' => array(
			array("id" => "_lp_skill_level", "type" => "text", "title" => esc_html__('Skill Level', 'starto-elementor' ), "description" => esc_html__('Enter Skill Level for this course ex. Beginner', 'starto-elementor' )),
			array("id" => "_lp_preview_video_embed", "type" => "textarea", "title" => esc_html__('Preview Video Embed Code', 'starto-elementor' ), "description" => esc_html__('Enter preview video HTML video embed code here', 'starto-elementor' )),
			
			array("id" => "_lp_what_learn", "type" => "WYSIWYG", "title" => esc_html__('What you\'ll learn', 'starto-elementor' ), "description" => esc_html__('Enter short detailed information about what customer will learn from this course', 'starto-elementor' )),
			
			array("id" => "_lp_course_template", "type" => "select", "title" => esc_html__('Course Template (Optional)', 'starto-elementor' ), "description" => esc_html__('Select course page template', 'starto-elementor' ), "items" => $theme_course_templates),
		),

);

function starto_create_meta_box() {
	...
	if ( function_exists('add_meta_box') && isset($postmetas) && count($postmetas) > 0 ) {  
		foreach($postmetas as $key => $postmeta)
		{
			if($_GET['post_type']==$key && !empty($postmeta))
			{
				...
				if($key == 'lp_course')
				{
					$option_name = esc_html__('Course', 'starto-elementor' );
				}
				
				add_meta_box( 'metabox', $option_name.' Options', 'starto_new_meta_box', $key, 'normal', 'high' );
			}
		}
	}
		...
		if($_GET['post_type'] == $key)
		{
			...
			foreach ( $postmeta as $postmeta_key => $each_meta ) {
				...
				if ($meta_type == 'checkbox') {
					...
					echo '<th width="5%"></th>';
					echo '<th width="90%">'.esc_html__("Title", 'starto-elementor').'</th>';
					echo '<th width="5%"></th>';
					...
					echo '<a id="'.$meta_id.'_add_row" class="button adding_list_add_row" data-target="'.$meta_id.'_sortable" data-metaid="'.$meta_id.'" href="javascript:;">'.esc_html__("Add another", 'starto-elementor').'</a>';
...
function starto_save_postdata( $post_id ) {
		...
		foreach ( $postmetas as $postmeta ) {
			foreach ( $postmeta as $each_meta ) {
				...
				if (isset($_POST[$each_meta['id']]) && $_POST[$each_meta['id']]) {
					starto_update_custom_meta($post_id, $_POST[$each_meta['id']], $each_meta['id']);
				}
...
function starto_update_custom_meta($postID, $newvalue, $field_name) {
...
add_action('admin_menu', 'starto_create_meta_box'); 
add_action('save_post', 'starto_save_postdata'); 	
?>