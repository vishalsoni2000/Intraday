...
if ( ! class_exists( 'Starto_Templates_Manager' ) ) {

	/**
	 * Define Starto_Templates_Manager class
	 */
	class Starto_Templates_Manager {
		 ...
		 * @var string
		 */
		protected $option = 'starto_templates';
		...
		public function init() {
			...
			if ( defined( 'ELEMENTOR_VERSION' ) && version_compare( ELEMENTOR_VERSION, '2.2.8', '>' ) ) {
				...
				add_action( 'wp_ajax_elementor_get_template_data', array( $this, 'force_starto_template_source' ), 0 );
			}
		}
		...
		public function register_templates_source() {
			...
			if($is_verified_envato_purchase_code)
			{
				/*if ( defined( 'ELEMENTOR_VERSION' ) && version_compare( ELEMENTOR_VERSION, '2.4.7', '>' ) ) {
					require STARTO_ELEMENTOR_PATH.'/templates_source_v_2_5.php';
				}
				else
				{
					require STARTO_ELEMENTOR_PATH.'/templates_source.php';
				}*/
				require STARTO_ELEMENTOR_PATH.'/templates_source.php';
				
				$elementor = Elementor\Plugin::instance();
				$elementor->templates_manager->register_source( 'Starto_Templates_Source' );
			}
		}
		...
		public function register_ajax_actions( $ajax ) {
			...
			if ( false === strpos( $data['template_id'], 'starto_' ) ) {
				return;
			}

			$ajax->register_ajax_action( 'get_template_data', array( $this, 'get_starto_template_data' ) );
		}

		/**
		 * Get starto template data.
		 *
		 * @param $args
		 *
		 * @return mixed
		 */
		public function get_starto_template_data( $args ) {

			$source = Elementor\Plugin::instance()->templates_manager->get_source( 'starto-templates' );
		 ...
		 * @return [type] [description]
		 */
		public function force_starto_template_source() {

			if ( empty( $_REQUEST['template_id'] ) ) {
				return;
			}

			if ( false === strpos( $_REQUEST['template_id'], 'starto_' ) ) {
				return;
			}

			$_REQUEST['source'] = 'starto-templates';
...
/**
 * Returns instance of Starto_Templates_Manager
 *
 * @return object
 */
function starto_templates_manager() {
	return Starto_Templates_Manager::get_instance();
}
